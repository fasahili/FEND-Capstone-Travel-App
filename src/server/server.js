...
app.post("/trip", async (req, res) => {
  try {
    ...
    const locationData = await getDataFromGeoNames(
    ...
    if (!locationData) {
      ...
      return res
        ...
        .send({
          success: false,
          message: "Failed to get coordinates from GeoNames",
        });
    }
    ...
    if (!weather) {
      console.error(
        "Failed to get weather data for coordinates:",
        locationData
      );
      return res
        .status(500)
        .send({
          success: false,
          message: "Failed to get weather data from Weatherbit",
        });
    }
